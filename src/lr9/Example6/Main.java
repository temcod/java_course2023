...
public class Main {
    ...
    private static Map<Integer, String> fillingMap() {
        ...
        for (int count = 0; count < Main.AMOUNT_WRITE; count++) {
            int key = random.nextInt(Main.MAX - Main.MIN + 1) + Main.MIN;
    ...
    private static String getRandomString(int size) {
        ...
        return IntStream.rangeClosed(1, size).
                mapToObj(i -> String.valueOf(Main.ENGLISH.charAt(random.nextInt(Main.ENGLISH.length())))).
                reduce("", (x, y) -> x + y);
    }
